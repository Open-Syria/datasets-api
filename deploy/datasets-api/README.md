...
- `.env` is private on the server and is not managed by this directory.
...
The deploy script runs migrations, syncs/imports the geography release, starts the inactive API slot, checks readiness, reloads nginx to the new slot, and stops the old slot after a short drain.

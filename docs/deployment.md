...
pnpm run release:check -- --geography-release v0.1.2
pnpm run db:migrate:deploy
DATASETS_RELEASE_SOURCES="Open-Syria/data-geography@v0.1.2" pnpm run datasets:sync
DATASETS_RELEASE_SOURCES="Open-Syria/data-geography@v0.1.2" DATABASE_ENABLED=true pnpm run read-model:import:geography
```

Use `pnpm run release:check:docker -- --geography-release v0.1.2` when Docker is available and the release job should also build the runtime image locally.
...
```bash
DATASETS_RELEASE_SOURCES="Open-Syria/data-geography@v0.1.2" pnpm run datasets:sync:prod
